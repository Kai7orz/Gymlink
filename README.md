...
これにより，実際の部屋の汚さは伝わりつつも，個人情報は保護できる共有アプリとなっています．加えて，部屋画像を共有する心理的なハードルを下げつつも，自身の部屋の汚さは公開されてしまうため，きれいな部屋を維持，きれいな部屋にしようとユーザーのモチベーションを向上させます．
...
ためです．（vueではなくNuxt にした理由は, ルーティングやページの遷移などを楽にしたかったため利用しました）

Golang: 
- 開発でVscode 上ですぐにミスに気づきやすい利点から静的型付け言語で開発をしたい
- Web 開発の最新情報が多い
...
- ユースケース（service）：一連の処理（本来ならサービスとドメインは分けなければならないが，今回は小規模なコードであるため，同じ場所にまとめた・今回は依存関係を明確にする構成で十分だと考えた）
...
### サービスとドメイン
- アプリのユースケースを記述（アプリの仕様上必要なバリデーション・データの登録などの動作の流れを記述）
